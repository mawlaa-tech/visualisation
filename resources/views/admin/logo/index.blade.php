...
<a href="{{route('add.logo')}}" class="btn btn-primary">
                            ...
                            @foreach($logos as $logo)
                              <tr>
                                <td scope="row"></td>
                                  ...
                                  <td><img src="{{asset($logo->image)}}" style="height:40px; width:70px;"></td>

                                  <td>
                                    @if($logo->created_at == NULL)
                                    <span class="text-danger">Pas de donnée</span>
                                    @else
                                    {{ Carbon\Carbon::parse($logo->created_at)->diffForHumans()}}</td>
                                    @endif
